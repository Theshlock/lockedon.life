...
var startupAnim = 1;
const startZoom = 300;
//const endZoom = 500000000000000;
const minZoom = 100;
//const maxZoom = Number.MAX_VALUE;
const maxZoom = 2000000000000000;
// console.log(maxZoom / endZoom);
...
//var endScreenX = -1.711354453;
//var endScreenY = -0.0000368837;
...
// Ultra high-resolution off-screen canvas for posters
...
// Zero-pad date/time fields for the poster file name
function pad( n ) 
{
	return n < 10 ? '0' + n : n;
}
...
var onJuliaComputeEnded = function (e) 
{
	...
	if( ! juliaRenderWorker ) {
		juliaRenderWorker = new Worker("julia-render.js");
		juliaRenderWorker.onmessage = onJuliaRenderEnded;
	}
...
var onJuliaRenderEnded = function (e)
{
	...
	mdJulia = new Uint8ClampedArray( e.data.pixelsBuffer );
	mJulia.data.set( mdJulia );
	juliaOffScreenCtx.putImageData( mJulia, 0,0 );
	jctx.drawImage( juliaOffScreen, 0,0, juliaCanvasWidth/2, juliaCanvasHeight/2 );

	// Draw crosshair over Julia image
	var x = xmouse * juliaCanvasWidth/8 + juliaCanvasWidth/4;
	var y = ymouse * juliaCanvasHeight/6 + juliaCanvasHeight/4;
	jctx.strokeStyle = "#FFFFFF";
	jctx.lineWidth = 2.0;
	jctx.beginPath();
	jctx.moveTo( x-5, y );
	jctx.lineTo( x+5, y );
...
{
	if( ! e.data.finished ) {
		// Not finished, just report progress
		if( blockSize[e.data.workerID] == 1 ) {
			...
			// Draw progress lines down sides of the canvas
	...
	var ultraSegmentCtx = ultraSegment.getContext("2d", { alpha: false } );
	...
	if( workersRunning == 0 ) {
		...
		// Convert the canvas to a PNG, then to a Blob that can have a short URL so we don't break Chrome
		...
		var theBlob = new Blob([ab], {type: mimeString});
...
function makePoster()
{
	...
	needRedraw = 1;
	document.getElementById("poster").innerHTML = "Cancel";
	...
	ultraCanvasCtx = ultraCanvas.getContext("2d", { alpha: false } );
	ultraCanvas.width  = ultraWidth;
	ultraCanvas.height = ultraHeight;

	ultraScaledCanvas = document.createElement('canvas');
	ultraScaledCanvasCtx = ultraScaledCanvas.getContext("2d", { alpha: false } );
	...
	for( i=0; i< workers; i++ ) {
		...
		computeWorker[i].postMessage({ mandelBuffer:ultraMandel[i].buffer, smoothMandel:ultraSmoothMandel[i].buffer, workerID:i, startLine:startLine, blockSize:1, canvasWidth:ultraWidth, segmentHeight: ultraHeight/workers, screenX:lscreenX, screenY:lscreenY, zoom:lzoom, iterations:iterations, smooth:smooth }, [ultraMandel[i].buffer], [ultraSmoothMandel[i].buffer]);
	}
}
...
var onRenderEnded = function (e) 
{
	...
	if( workersRunning == 0 ) {
		...
		if(( ! eventOccurred ) && ( finished[0] ) && ( finished[1] ) && ( finished[2] ) && ( finished[3] )) {
			...
			//offScreenCtx.putImageData( mSegment[workerID], 0,0, 0,lstartLine, canvasWidth,chunkHeight );
			//mctx.drawImage(offScreen, 0, 0, canvasWidth/scaleFactor, canvasHeight/scaleFactor);
		}
	}
	if( showAxes )
	...
	//var ldiff = lend - lstart;
	//console.log("onWorkEnded: "+ldiff);
};

function wheelMoved(event) 
{
	// Don't scroll if we're inside the Canvas
	...
	if( ( mx <= 0 ) || ( mx > canvasWidth ) || ( my <= 0 ) || ( my > canvasHeight ) )
		return 1;
/*	mctx.strokeStyle="rgb("+Math.floor(Math.random()*255)+","+Math.random()*255+","+Math.random()*255+")";
	mctx.beginPath();
	mctx.moveTo( mx/scaleFactor, 0 );
	mctx.lineTo( mx/scaleFactor, rect.height );
	mctx.stroke();
	mctx.beginPath();
	mctx.moveTo( 0, my/scaleFactor );
	mctx.lineTo( rect.width, my/scaleFactor );
	mctx.stroke();
*/
	...
	// Compute inverse transformation to keep correct pixel under the pointer
...
function touchStart(event)
{
	...
	//mctx.fillRect( m1x/scaleFactor, m1y/scaleFactor, 20, 20 );
	//alert(touches.length + ' ' + touches[0].id + ' ' + touches[0].x + ' ' + touches[0].y );
}
...
function touchMove(event)
{
	...
	if( event.targetTouches.length >= 2 ) {
		...
		if( ( m1x <= 0 ) || ( m1x > canvasWidth ) || ( m1y <= 0 ) || ( m1y > canvasHeight ) ||
		    ( m2x <= 0 ) || ( m2x > canvasWidth ) || ( m2y <= 0 ) || ( m2y > canvasHeight ) )
			return 1;
		...
		//mctx.fillRect( centerX/scaleFactor, centerY/scaleFactor, 20, 20 );

		//var deltaX = (m1x+m2x)/2 - canvasCenterX;
		//var deltaY = (m1y+m2y)/2 - canvasCenterY;
		...
		if( firstPinchDistance != 0 ) {
			...
			if( zoom < minZoom )
				zoom = minZoom;
			// Compute inverse transformation to keep correct pixel at the center of the pinch
...
function drawZoomButtons( redraw, highlight )
{
	...
	//mctx.fillText( "+", zoomInBtn.x+6, zoomInBtn.y+25 );
	//mctx.fillText( "-", zoomOutBtn.x+6, zoomOutBtn.y+23 );
	mctx.stroke();	
...
{
	// Update Re/Im co-ordinates summary box
	...
	//var section = Math.floor( y * workers / canvasHeight );
	//if( section >= workers )
	//	section = workers-1;
	//var itersToPrint = mandel[section][Math.floor(x + (y - section * canvasHeight / workers) * canvasWidth)];
	coordSourceText.textContent = " " + source;
	coordSource2Text.textContent = " " + source;
	oneShotWorker.postMessage( { oneShot:1, x:x, y:y, screenX:screenX, screenY:screenY, zoom:zoom, iterations:iterations, smooth:0 } );
	if(( showJulia ) && ( ! drawingJulia )) 
...
function setup() 
{
	...
	eventTime = performance.now(); 
	//zoomSlider = createSlider( 100, maxZoom, 250 );
	//document.getElementById("mandelCanvas").addEventListener("mousedown", moveCanvas, false);
...
function drawMandel() 
{
	...
	if(( showJulia ) && ( juliaIterations != juliaIterSlider.value )) {
		juliaIterations = Math.floor(juliaIterSlider.value);
		juliaItersText.textContent = juliaIterations;
		updateCoords( canvasWidth/2, canvasHeight/2, "centre" );
	}
...
contextM.fillStyle = 'red';

contextB = mc.getContext('2d');
contextB.fillStyle = 'white';

window.requestAnimationFrame(gameloop);
function gameloop() {
	contextM.fillRect( ( (portalX-xnorm) * zoom + 800) / 2, ( (portalY-ynorm) * zoom + 600) / 2, 20+zoom/portalDepth*1000, 20 + zoom/portalDepth*1000 );
	contextM.fillRect( ( (portalX-xnorm) * zoom + 800) / 2, ( (portalY-ynorm) * zoom + 600) / 2, 25+zoom/portalDepth*1000, 25 + zoom/portalDepth*1000 );
	xRate += ( right - left ) / 10 * ( Date.now() - time ) / 10;
	yRate += ( down - up ) / 10 * ( Date.now() - time ) / 10;
	xnorm += ( xRate / zoom ) * ( Date.now() - time)  / 10;
	ynorm += ( yRate / zoom ) * ( Date.now() - time ) / 10;
	speed += ( shift - spacebar) * ( Date.now() - time ) / 10;
        zoom += Math.ceil( zoom / 20000 ) * speed * ( Date.now() - time );
	time = Date.now();
        if( zoom > portalDepth ) {
		zoom = 1;
		speed = 1;
