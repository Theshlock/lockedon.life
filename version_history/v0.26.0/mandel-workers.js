...
window.onkeydown = function(event) {
    ...
    if (event.keyCode === 16) {shift = 1}; //shift arrow
    if (event.keyCode === 32) {spacebar = 1}; //spacebar arrow
};
...
window.onkeyup = function(event) {
    ...
    if (event.keyCode === 16) {shift = 0}; //shift arrow
    if (event.keyCode === 32) {spacebar = 0}; //spacebar arrow
};
...
function gameloop() {
	if (status == "main menu") {
		...
		if (Date.now() - startTime < 3000) {contextM.fillText("Use the Arrow keys, shift and spacebar to control", 100, 500)};
